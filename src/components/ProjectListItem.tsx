...
export const ProjectListItem = ({ name, category, image, link }: ProjectListItemProps) => {
  ...
  return (
    <a
      href={link}
      className="group block border-b border-gray-900/90 dark:border-white/90 py-8 
                relative"
    ...
    >
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-4">
          ...
          />
          <h2 className="font-display text-3xl text-gray-900 dark:text-white/90
                       transform transition-transform duration-300 -translate-x-4
                       group-hover:translate-x-0">
            {name}
          </h2>
        </div>
        <span className="text-right text-lg text-gray-600 dark:text-white/70
                      transform transition-transform duration-300 translate-x-4
                      group-hover:translate-x-0">
          {category}
        </span>
      ...
      {/* Thumbnail Preview */}
      <div 
        className={`fixed left-0 top-36 w-[45vw] h-[60vh] pointer-events-none z-50
